...
class ImuLidarTransformNode(Node):
    IMU_FRAME = "imu"
    LIDAR_FRAME = "os_lidar"

    def __init__(self):
        super().__init__("imu_lidar_transform_node")
        self.subscription = self.create_subscription(
            Imu, "/imu/data", self.imu_callback, 10
        )
        self.publisher = self.create_publisher(Imu, "/imu/data/transformed", 10)
    ...
    def transform_imu(self, imu_msg):
        try:
            tf = self.tf_buffer.lookup_transform(
                self.LIDAR_FRAME, self.IMU_FRAME, imu_msg.header.stamp
            )
        ...
        # self.get_logger().debug(f"tf_rot: {tf_rot}")

        # Transform the IMU angular velocity
        imu_ang_vel_transformed = self._tf_vector(tf_rot, imu_msg.angular_velocity)

        # Transform the IMU linear acceleration
        imu_lin_acc_transformed = self._tf_vector(tf_rot, imu_msg.linear_acceleration)
    ...
    def _tf_vector(self, tf: np.ndarray, vec: Vector3) -> Vector3:
        vec_arr = np.array([vec.x, vec.y, vec.z])
        vec_transformed = tf.dot(vec_arr)

        return Vector3(x=vec_transformed[0], y=vec_transformed[1], z=vec_transformed[2])
    

    # def _tf_angular(self, tf: np.ndarray, vec: Vector3) -> Vector3:
    #     tf_ang = R.from_euler('xyz', [0, np.pi, 0], degrees=False).as_matrix()
    #     tf_new = tf @ tf_ang

    #     vec_arr = np.array([vec.x, vec.y, vec.z])
    #     vec_transformed = tf_new.dot(vec_arr)
...
def main(args=None):
    ...
    imu_transform_node = ImuLidarTransformNode()
