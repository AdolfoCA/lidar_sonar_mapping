...
Subscribes to:
  - Sonar point cloud (from acoustic3d_edge, frame: oculus)
...
class SonarScanNode(Node):
    ...
    def __init__(self):
        ...
        self.sonar_sub = self.create_subscription(
            PointCloud2,
            'sonar_cloud_horizontal',  # From acoustic3d_edge, frame: oculus
            self.sonar_callback,
            10
        )
        ...
        self.cloud_pub = self.create_publisher(
            PointCloud2,
            'sonar_map_cloud',  # Output in odom frame
            10
        )
        ...
        self.get_logger().info('  - sonar_cloud_horizontal (frame: oculus)')
    ...
    def sonar_callback(self, msg: PointCloud2):
        ...
        # msg.header.frame_id should be 'oculus'
        try:
        ...
        # Static transform: os_imu -> oculus

        # Same orientation, sonar is 43 cm below IMU
        T_imu_oculus = np.eye(4)
        T_imu_oculus[2, 3] = -0.43  # z = -43 cm (down in FLU)
        ...
        T_odom_oculus = T @ T_imu_oculus

        # Transform points
        transformed_xyz = (T_odom_oculus @ xyz_homogeneous.T).T[:, :3]
