...
class ProbSonarMapNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('window_radius', 20000.0)
        ...
        # Mission log: parameter can be a directory or a full .csv path.
        # If a directory is given, the file is named mission_log.csv inside it.
        # If empty, default to the same directory as save_path.
        log_path = str(gp('mission_log_path'))
        if not log_path and self.save_path_:
            log_path = os.path.dirname(self.save_path_)
        if log_path and not log_path.endswith('.csv'):
            log_path = os.path.join(log_path, 'mission_log.csv')
        self.mission_log_path_ = log_path

        # (elapsed_s, scan_count, voxel_count)
        self._voxel_log_: List[Tuple[float, int, int]] = []
        self._node_start_time_ = time.monotonic()
    ...
    def _save_mission_log(self) -> None:
        """Write voxel-count-over-time log to CSV."""
        if not self.mission_log_path_ or not self._voxel_log_:
            return
        try:
            os.makedirs(
                os.path.dirname(os.path.abspath(self.mission_log_path_)),
                exist_ok=True)
            with open(self.mission_log_path_, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['elapsed_s', 'scan_count', 'voxel_count'])
                writer.writerows(self._voxel_log_)
            self.get_logger().info(
                f'Mission log saved: {len(self._voxel_log_)} entries → '
                f'{self.mission_log_path_}')
        except Exception as e:
            self.get_logger().error(f'Mission log save failed: {e}')
    ...
    def cloud_callback(self, msg: PointCloud2):
        ...
        self._scan_count_ += 1
        elapsed = t0 - self._node_start_time_
        self._voxel_log_.append((round(elapsed, 3), self._scan_count_, len(self.voxel_map_)))
