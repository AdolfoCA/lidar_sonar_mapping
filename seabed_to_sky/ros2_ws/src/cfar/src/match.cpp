...
Eigen::MatrixXf matchFeatures(const pybind11::array_t<int>& range_horizontal_discrete,
    ...
    vector<int> horizontal_indices = {};
    vector<int> vertical_indices = {};
    ...
    for (auto & subproblem : horizontal_bins){
      ...
      while(i < best_vector.size() && i < vertical_problem.size()){
        ...
        horizontal_indices.push_back(j);
        vertical_indices.push_back(k);
        i += 1;
      }
    }
    ...
    Eigen::MatrixXf output_matrix(range_horizontal_matched.size(),6);
    ...
    float * ptr_5 = &bearing_vertical_matched[0];
    Eigen::Map<Eigen::VectorXf> col_5(ptr_5, bearing_vertical_matched.size());

    float* ptr_6 = &matching_confidence[0];
    Eigen::Map<Eigen::VectorXf> col_6(ptr_6, matching_confidence.size()); 
